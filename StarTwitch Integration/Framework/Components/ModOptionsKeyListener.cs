...
namespace StarTwitch_Integration.Framework.Components
{
    ...
    internal class ModOptionsKeyListener : BaseOptionsElement
    {
        /*********
        ** Fields
        *********/
        ...
        *********/
        ...
        public ModOptionsKeyListener(string label, int slotWidth, SButton value, Action<SButton> setValue, SButton clearToButton = SButton.None)
          : base(label, -1, -1, slotWidth + 1, 11 * Game1.pixelZoom)
        {
            Value = value;
            PressNewKeyLabel = I18n.Controls_Pressnewkey();
            SetValue = setValue;
        ...
        {
            if (greyedOut || IsListening || !SetButtonBounds.Contains(x, y) || Constants.TargetPlatform == GamePlatform.Android)
                return;
        ...
        public override void ReceiveButtonPress(SButton button)
        {
            ...
            if (InvalidButtons.Contains(button))
            {
                Value = ClearToButton;
        ...
        public override void draw(SpriteBatch spriteBatch, int slotX, int slotY, IClickableMenu? context = null)
        {
            ...
            if (IsListening)
            {
                ...
                spriteBatch.DrawString(Game1.dialogueFont, PressNewKeyLabel, Utility.getTopLeftPositionForCenteringOnScreen(Game1.tileSize * 3, Game1.tileSize), Color.White, 0.0f, Vector2.Zero, 1f, SpriteEffects.None, 0.9999f);
            }
        }
    }
}
